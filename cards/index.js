// ViewRepo Card — GitHub Action entry point.
// Reads ViewRepo's analyzer out of index.html, runs it on the consuming repo,
...
function log(msg) {
  process.stdout.write('[ViewRepo-card] ' + msg + '\n');
}
...
run().catch((err) => {
  process.stderr.write('[ViewRepo-card] error: ' + (err.stack || err.message || err) + '\n');
  process.exit(1);
});