...
function loadInputs() {
  const output = readInput('output', '.github/viewrepo-card.svg');
  const state = readInput('state', '.github/viewrepo-card.json');
  ...
  const commitMessage = readInput('commit-message', 'chore: update viewrepo card [skip ci]');
  const commitAuthorName = readInput('commit-author-name', 'viewrepo-card[bot]');
  const commitAuthorEmail = readInput(
    'commit-author-email',
    'viewrepo-card[bot]@users.noreply.github.com'
  );
