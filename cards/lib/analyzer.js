// Extract the viewrepo analyzer block from index.html and run it in a Node vm
// context. Mirrors what tests/VIEW REPO-golden.test.mjs does — the analyzer is
...
const START_MARKER = '// ===== VIEW REPO_ANALYZER_START =====';
const END_MARKER = '// ===== VIEW REPO_ANALYZER_END =====';
const METRICS_START = '// ===== VIEW REPO_METRICS_START =====';
const METRICS_END = '// ===== VIEW REPO_METRICS_END =====';
...
function loadAnalyzer(htmlPath) {
  ...
  vm.runInContext(analyzerSource + '\n' + metricsSource + exposeExports, context, {
    filename: 'VIEW REPO-analyzer.js',
  });
...
function locateIndexHtml(actionDir, repoRoot) {
  // 1) Adjacent to the Action (same repo as VIEW REPO itself):
  const adjacent = path.join(actionDir, '..', 'index.html');
  if (fs.existsSync(adjacent)) return adjacent;
  // 2) Repo root (when the Action is run from inside VIEW REPO's checkout):
